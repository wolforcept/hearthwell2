...
public class EntityHearthWell extends Entity {
	...
	private static final EntityDataAccessor<CompoundTag> UNLOCKED_NODES = SynchedEntityData.defineId(EntityHearthWell.class, EntityDataSerializers.COMPOUND_TAG);
...
//				ent.setDefaultPickupDelay();
				int particleColor = hasItems() ? HearthWell.getRandomColorOfHearthwell() : 0xFFFF0000;
				level.addParticle(new ParticleEnergyData(particleColor), entPos.x, entPos.y + 0.5, entPos.z, 0, 0, 0);
			}
...
//			level.addParticle(new ParticleEnergyData(0), x, y + .5 + random.nextFloat() * .5, z, //
					random.nextGaussian() * .0025, random.nextGaussian() * .0075, random.nextGaussian() * .0025);
		}
		...
		for (ItemEntity itemEntity : nearItemsGood) {
			...
			if (inputStack != null && entityStack.getCount() >= inputStack.getCount()) {
				...
				((ServerLevel) level).sendParticles(//
						new ParticleEnergyData(), itemPos.x, itemPos.y, itemPos.z, 20, 0, 0, 0, 0.01);
				return itemPos;
			}
		}
		return null;
...
//								((ServerLevel) level).sendParticles(new ParticleEnergyData(0xFFFFFF), pos.x, pos.y + 0.66, pos.z, 40, 0,
...
//				}
				...
				setHasItems(false);
				((ServerLevel) level).sendParticles(new ParticleEnergyData(0xFFFF0000), //
						xOld, yOld + 0.76, zOld, // position
						2, // particleCount
						0, random.nextGaussian() * .5, 0, // offset
						0.005 // speed
				);
			}
		}
