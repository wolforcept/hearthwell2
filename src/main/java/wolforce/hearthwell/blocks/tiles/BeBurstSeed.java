...
public class BeBurstSeed extends BlockEntityParent {
	...
	public BeBurstSeed(BlockPos worldPosition, BlockState blockState) {
...
//		int count = stack.getCount() * stack.getCount() / 4;
	...
	public ItemStack tryAddItem(Player player, ItemStack stack) {
		...
		if (!UtilItemStack.isValid(item) || (UtilItemStack.equalExceptAmount(item, stack) && item.getCount() < 64)) {
