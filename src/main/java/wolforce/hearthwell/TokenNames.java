...
//	}
	...
	private static final String[] premadeWords = { "Abarca", "Abhor", "Abuse", "Access", "Ace", "Acid", "Aid", "Air", "Almeiz", "Anyo", "Arco", "Awe", "Bank", "Bea", "Belief", "Beluga", "Bery", "Bin",
			"Blood", "Carbon", "Change", "Click", "Cold", "Colt", "Cue", "Damage", "Dans", "Dee", "Depth", "Desire", "Dust", "Ease", "Effort", "Emir", "End", "Ent", "Epht", "Erdzo", "Error", "Excess",
			"Exit", "Fael", "Fate", "Few", "Fiber", "Fire", "Flying", "Fold", "Forest", "Ghast", "Glass", "Go", "God", "Gold", "Ground", "Growth", "Hand", "Harata", "Haste", "Hay", "Health", "Hero",
			"Hymn", "Ice", "Imake", "Iner", "Injury", "Intent", "Ira", "Ira", "Iron", "Jack", "Japher", "Jem", "Jhad", "Jhast", "Joy", "Junipe", "Ka", "Kauer", "Keep", "Kesh", "King", "Kneiss", "Ko",
			"Krones", "Labour", "Lag", "Lead", "Leaf", "Lift", "Light", "Locus", "Lowers", "Luck", "Magic", "Maithe", "Malren", "Mass", "Mat", "Maze", "Meap", "Methil", "Mezzos", "Mil", "Moon",
			"Nobody", "Noneva", "Nonne", "North", "Oak", "Obtus", "Ocean", "Oghein", "Opssum", "Opus", "Ore", "Origin", "Pace", "Pain", "Palus", "Peace", "Plenty", "Profit", "Py", "Qhan", "Quasar",
			"Quiet", "Quins", "Qwanda", "Qwinciat", "Rag", "Rain", "Range", "Reaver", "Recall", "Rock", "Rog", "Rose", "Rumus", "Scent", "Sea", "Self", "Set", "Shock", "Sky", "Snow", "Stars",
			"Stream", "System", "Tar", "Tardaz", "Tech", "Text", "Thar", "Thobru", "Time", "Times", "Tink", "Tokyen", "Travel", "Trees", "Umesmo", "Union", "Up", "Ur", "Uszi", "Uyglas", "Verita",
			"Vex", "Vi", "Vira", "Voice", "Volume", "Vurze", "Wannyr", "War", "Ward", "Wave", "Wealth", "Wing", "Wood", "Wound", "Wright", "Xandar", "Xis", "Xylia", "Yar", "Yelena", "Yen", "Yield",
			"Yma", "Yn", "Yonder", "Zaun", "Zero", "Zest", "Zod", "Zombie" };

	@SuppressWarnings("rawtypes")
	private static final List[] wordsBySize = { //
			Arrays.stream(premadeWords).filter(x -> x.length() == 2).toList(), Arrays.stream(premadeWords).filter(x -> x.length() == 3).toList(),
			Arrays.stream(premadeWords).filter(x -> x.length() == 4).toList(), Arrays.stream(premadeWords).filter(x -> x.length() == 5).toList(),
		...
		for (int i = 0; i < size; i++) {
			if (a.charAt(i) != b.charAt(i)) {
				if (i == size - 1 && n + 1 + Math.abs(a.length() - b.length()) < min)
					return true;
				if (i < size - 1 && n + 1 < min)
					return distance(a.substring(i + 1), b, n + 1, min) || distance(a, b.substring(i + 1), n + 1, min) || distance(a.substring(i + 1), b.substring(i + 1), n + 1, min);
				return false;
			}
		}
...
//		return chars[new Random(world.getBiomeManager().biomeZoomSeed + (blockpos.getX() + "" + blockpos.getY() + "" + blockpos.getZ()).hashCode()).nextInt(chars.length)];
	}

	public static String getNamesOfPos(BlockPos onPos) {
		char c = TokenNames.getLetterOfPos(onPos);
		List<? extends String> names = ConfigServer.getTokenNames();
		String namesWithLetter = "";
		for (int i = 0; i < names.size(); i++) {
			if (names.get(i).toLowerCase().contains((c + "").toLowerCase())) {
				namesWithLetter += createSingleName(i);
			}
		}
		return namesWithLetter;
	}

	public static boolean containsNameIndex(String names, int n) {
		return names.contains("." + n + ".");
	}

	public static String createSingleName(int n) {
		return "." + n + ".";
	}
	...
	public static int getColorOfToken(int fuelType) {
		...
		if (fuelType < 0 || fuelType >= tokenColours.length)
			return 0;

		return tokenColours[fuelType];

	}

//	public static int getFuelTypeOfPos(BlockPos onPos) {
//		return 0;
//	}
